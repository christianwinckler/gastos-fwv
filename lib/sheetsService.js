...
export async function updateParametros(rows) {
  ...
  const sheets = await getSheets()
  const estadosRes = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.SHEET_ID,
    range: 'Parámetros!A2:E500',
  })
  const estadosMap = {}
  ;(estadosRes.data.values || []).forEach(r => {
    if (r[0]) estadosMap[r[0].trim()] = r[4] || ''
  })
  const rowsConEstado = rows.map(r => {
    const sub = (r[0] || '').trim()
    const estadoNuevo = r[4] || ''
    const estadoExistente = estadosMap[sub] || ''
    return [r[0], r[1], r[2], r[3], estadoNuevo || estadoExistente]
  })
  ...
  await sheets.spreadsheets.values.append({
    ...
    range: 'Parámetros!A2',
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: rowsConEstado }
  })
}
