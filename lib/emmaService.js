...
// ════════════════════════════════════════
// REGISTRO DIARIO
// Columnas: Fecha|Hora|TipoRegistro|Detalle|Cantidad|Unidad|Estado|Nota  (A:H)
// ════════════════════════════════════════
...
export async function addRegistro({
  fecha, hora = '', tipoRegistro, detalle = '', cantidad, unidad = '',
  estado, nota = ''
}) {
  const sheets = await getSheetsClient()
  const colRes = await sheets.spreadsheets.values.get({
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[
        fecha, hora, tipoRegistro, detalle,
        cantidad, unidad, estado, nota
      ]]
    }
  })
  return newRowIndex
...
export async function updateRegistro(rowIndex, {
  ...
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SHEET_ID(),
    range: `RegistroDiario!A${rowIndex}:H${rowIndex}`,
  })
